...
def load_checkpoint(checkpoint_version):
    ...
    print("\n checkpoints available are  : " , dirs)
    if len(dirs) > 1:
        # dirs.remove("best-checkpoint.ckpt")
        best_checkpoint = max(dirs)
        print("\n from multiple checkpoints loading checkpoint : " , best_checkpoint)
    else:
        best_checkpoint = dirs[0]
        print("\n only one checkpoint found , loading checkpoint : " , best_checkpoint)
